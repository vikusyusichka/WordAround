            ...
            VStack(spacing: 5) {
                ZStack {
                    Circle()
                        ...
                        .frame(width: Layout.convMicSideButtonSize, height: Layout.convMicSideButtonSize)
                    Image(systemName: isLastPhrase ? "flag.checkered" : "arrow.right")
                        .font(.system(size: Layout.convMicSideIconSize, weight: .semibold))
                        .foregroundColor(accent)
                }
                Text(isLastPhrase ? "Finish" : "Next")
                    .font(.system(size: 11, weight: .semibold, design: .rounded))
                    .foregroundColor(accent)
            }
        }
...
#Preview {
    ZStack {
        ...
        VStack(spacing: 16) {
            Spacer()
            ShadowingMicBarView(state: .idle, canRetry: false, isLastPhrase: false, onRetry: {}, onMic: {}, onNext: {})
            ShadowingMicBarView(state: .listening, canRetry: true, isLastPhrase: false, onRetry: {}, onMic: {}, onNext: {})
            ShadowingMicBarView(state: .processing, canRetry: true, isLastPhrase: true, onRetry: {}, onMic: {}, onNext: {})
        }
