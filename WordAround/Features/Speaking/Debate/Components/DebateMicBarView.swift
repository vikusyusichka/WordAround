...
struct DebateMicBarView: View {
    let state: SpeakingConversationState
    let onEnd: () -> Void
    let onMic: () -> Void
    let onHint: () -> Void

    private let pink = DebateTheme.accent
        ...
        Button(action: onMic) {
            ZStack {
                if state.isListening {
                    Circle()
                        .stroke(pink.opacity(0.20), lineWidth: 9)
    ...
    private var micCircleColor: Color {
        switch state {
        case .idle, .error: return pink
        case .listening:    return Color(red: 0.93, green: 0.22, blue: 0.22)
        case .processing:   return Color(red: 0.95, green: 0.62, blue: 0.78)
        case .speaking:     return AppColors.greenAccent
        }
    }
}
...
#Preview {
    ZStack {
        ...
        VStack(spacing: 16) {
            Spacer()
            DebateMicBarView(state: .idle, onEnd: {}, onMic: {}, onHint: {})
            DebateMicBarView(state: .listening, onEnd: {}, onMic: {}, onHint: {})
            DebateMicBarView(state: .processing, onEnd: {}, onMic: {}, onHint: {})
        }
